...
\ This one is added word-by-word as they are successfully executed
\ Most of the comments are taken directly from the JonesForth Forth code that I'm reproducing
\ here and updating to be correct for a 64-bit Forth
\ 
\   START OF FORTH CODE ----------------------------------------------------------------------
\
...
\ x86, the idivl instruction gives both anyway).  Now we can define the / and MOD in terms of /MOD
...
\ Here we're defining a literal colon character
: ':'
        [
        CHAR :
        ]
...
: DO IMMEDIATE
        ' (DO) ,        \ Compile the runtime primitive
        HERE @          \ Push current address (target for jump)
;

: LOOP IMMEDIATE
        ' (LOOP) ,      \ Compile runtime primitive
        HERE @ -        \ Calculate Offset: Target (from stack) - Current HERE
        ,               \ Compile the 64-bit offset
;

: +LOOP IMMEDIATE
        ' (+LOOP) ,     \ Compile runtime primitive
        HERE @ -        \ Calculate offset
        ,               \ Compile the 64-bit offset
;
...
\ Writes N spaces to stdout using our looping constructs :-)
: SPACES ( n -- )
    BEGIN
        DUP 0>
    WHILE
...
: ZU.R ( u width -- )
    ...
    ROT
    SWAP -
    ( Same as above - ZEROS won't print a negative number of zeros... )
    ZEROES
    U.
;
    ...
    ?DUP IF         ( only act if the exception code <> 0 )
        ...
        WHILE
            ...
            ' EXCEPTION-MARKER 8+ = IF  ( found the EXCEPTION-MARKER on the return stack )
                ...
                DSP!
                R> ITRRSP!  ( restore the iterator stack ) 
                EXIT       ( restore the parameter stack pointer, immediately exit )
            THEN
            8+
        REPEAT
